 ...
 * Captures the URL value without quotes
 */
...
const processStyleUrls = (styleValue, hostnames, opts, debug) => {
  ...
  const updatedStyle = styleValue.replace(CSS_URL_REGEX, (match, quote, url$1) => {
    if (!url$1 || typeof url$1 !== 'string') {
      return match;
    }
    debug('Processing CSS url(): %s', url$1);

    // Skip data URLs, hash fragments, and other special URLs
    if (url$1.startsWith('data:') || url$1.startsWith('#')) {
      debug('Skipping special CSS URL: %s', url$1);
      return match;
    }

    // Handle relative URLs that start with / (root-relative)
    if (url$1.startsWith('/') && !url$1.startsWith('//') && opts.basePath) {
      localCount++;
      hasChanges = true;
      const newUrl = `/${opts.basePath}${url$1}`;
      debug('Converting relative CSS url: %s to %s', url$1, newUrl);
    ...
    try {
      ...
      if (urlData.protocol && urlData.hostname) {
        // Check if hostname is in our "local" list
        if (hostnames.has(urlData.hostname)) {
          // Strip protocol and hostname from local URL, prepend base path if provided
          localCount++;
          hasChanges = true;
          const pathWithQuery = (urlData.path || urlData.pathname || '') + (urlData.hash || '');
          const newUrl = opts.basePath ? `/${opts.basePath}${pathWithQuery}` : pathWithQuery;
          debug('Converting local CSS url: %s to %s', url$1, newUrl);
          return `url(${quote}${newUrl}${quote})`;
        } else {
          // External URLs in CSS don't get target/rel attributes (not applicable to CSS)
          externalCount++;
          debug('Found external CSS url: %s', url$1);
        }
      }
    } catch (err) {
      debug('Error parsing CSS URL %s: %s', url$1, err.message);
    }
    return match;
  });
  return {
